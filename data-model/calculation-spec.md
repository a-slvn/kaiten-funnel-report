...
### 9.16. Доска с одной колонкой (non-done) — 0 этапов после Best Guess

- **Сценарий:** Пространство содержит 1 доску с единственной колонкой типа `in_progress` (или `queue`). Колонок с `column_type = 'done'` нет.
- **Поведение Best Guess:** Алгоритм собирает 1 stage. Затем обнаруживает 0 done-колонок → выполняет `stages.pop()`, назначая единственный этап как won-колонку. Результат: `stages = []`, `win_column_ids = [column_id]`, `loss_column_ids = []`.
- **Влияние на расчёты:**
  - 0 этапов → CTE `stage_visits` возвращает пустой набор строк
  - `funnel_deal_history` — ни одной записи (нет этапов для привязки)
  - `funnel_stage_snapshot` — пустая таблица
  - `funnel_deal_summary` — все сделки получают `outcome = 'in_progress'` (нет этапов, нет переходов)
  - Все метрики (`deals_entered`, `conversion_to_next`, `conversion_to_win`, `drop_off_rate`, `avg_duration_days`) = `null` или `0`
  - `total_entered = 0`, `total_won = 0`, `total_lost = 0` в summary
- **Отображение:** Empty state. Воронка не строится. Сообщение: «Недостаточно этапов для построения воронки. Добавьте колонки на доску.»
- **Confidence:** `low`. Алерт: `NO_DONE_COLUMNS`.
- **Отличие от 9.13:** В 9.13 (несколько non-done колонок, нет done) после pop остаются этапы. Здесь единственная колонка — этапов не остаётся.
- **Тест-кейс:** TC-41
